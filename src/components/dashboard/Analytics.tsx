...
import { TrendingUp, ShoppingCart, Star, Users, Activity, Calendar, Target, Award, Zap, Eye, Heart, Sparkles } from 'lucide-react';
...
const StatCard = ({ 
  ...
  >
    ...
    <div className="absolute -top-8 -right-8 w-32 h-32 bg-gradient-to-br from-blue-400/20 to-purple-600/20 rounded-full blur-xl group-hover:scale-150 transition-all duration-1000"></div>
    <div className="absolute -bottom-8 -left-8 w-24 h-24 bg-gradient-to-tr from-pink-400/20 to-orange-600/20 rounded-full blur-xl group-hover:scale-125 transition-all duration-1000 delay-200"></div>
    ...
    <div className="absolute inset-0 bg-gradient-to-br from-white/10 via-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 backdrop-blur-sm"></div>
    
    <div className="relative z-10">
      <div className="flex items-center justify-between mb-6">
        <div className={`relative w-16 h-16 ${gradient || bgColor} rounded-2xl flex items-center justify-center shadow-2xl group-hover:shadow-3xl transition-all duration-500 group-hover:rotate-12 group-hover:scale-110 transform-gpu`}>
          {/* Icon Glow Effect */}
    ...
    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-1200 ease-out"></div>
    
    {/* 3D Border Effect */}
    <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-white/20 via-transparent to-black/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
  </div>
);
...
const ChartCard = ({ 
  ...
  >
    {/* Animated Background */}
    <div className="absolute inset-0 bg-gradient-to-br from-blue-50/80 via-purple-50/40 to-pink-50/60 opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
    
    {/* Floating Orbs */}
    <div className="absolute -top-12 -right-12 w-40 h-40 bg-gradient-to-br from-blue-400/10 to-purple-600/10 rounded-full blur-2xl group-hover:scale-150 transition-all duration-1000"></div>
    <div className="absolute -bottom-12 -left-12 w-32 h-32 bg-gradient-to-tr from-pink-400/10 to-orange-600/10 rounded-full blur-2xl group-hover:scale-125 transition-all duration-1000 delay-300"></div>
    ...
    <div className="relative z-10">
      ...
      <div className="group-hover:scale-[1.01] transition-transform duration-500 transform-gpu">
        {children}
      </div>
    </div>
...
const MetricCard = ({ 
  ...
  >
    {/* Background Gradient */}
    <div className="absolute inset-0 bg-gradient-to-br from-gray-50/50 to-white/50 group-hover:from-blue-50/50 group-hover:to-purple-50/50 transition-all duration-500"></div>
    
    <div className="relative z-10 flex items-center justify-between">
      <div className="flex-1">
        <p className="text-sm font-medium text-gray-600 mb-1">{label}</p>
        <p className={`text-2xl font-bold ${color} group-hover:scale-105 transition-transform duration-300 origin-left`}>
          {value}
        </p>
      </div>
      <div className={`w-12 h-12 bg-gradient-to-br ${color.includes('emerald') ? 'from-emerald-400 to-emerald-600' : color.includes('blue') ? 'from-blue-400 to-blue-600' : 'from-purple-400 to-purple-600'} rounded-xl flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all duration-300 group-hover:rotate-12 group-hover:scale-110`}>
...
export default function DashboardAnalytics() {
  ...
  const calculateMetrics = (analyticsData: AnalyticsData) => {
    ...
    const ordersGrowth = ordersChart.length >= 2 
      ? ((ordersChart[ordersChart.length - 1]?.count - ordersChart[ordersChart.length - 2]?.count) / ordersChart[ordersChart.length - 2]?.count * 100)
      : 0;
    
    const reviewsGrowth = reviewsChart.length >= 2
      ? (((reviewsChart[reviewsChart.length - 1]?.positive + reviewsChart[reviewsChart.length - 1]?.negative) - 
          (reviewsChart[reviewsChart.length - 2]?.positive + reviewsChart[reviewsChart.length - 2]?.negative)) / 
          (reviewsChart[reviewsChart.length - 2]?.positive + reviewsChart[reviewsChart.length - 2]?.negative) * 100)
      : 0;
    
    // Calculate conversion rate (reviews/orders)
    const conversionRate = totalOrders > 0 ? (totalReviews / totalOrders * 100) : 0;
    
    // Calculate average orders per month
    const avgOrdersPerMonth = ordersChart.length > 0 
      ? ordersChart.reduce((sum, item) => sum + item.count, 0) / ordersChart.length
      : 0;
    
    // Calculate average reviews per month
    const avgReviewsPerMonth = reviewsChart.length > 0 
      ? reviewsChart.reduce((sum, item) => sum + item.positive + item.negative, 0) / reviewsChart.length
      : 0;
    
    // Find best performing month
    const bestMonth = ordersChart.length > 0 
      ? ordersChart.reduce((max, item) => item.count > max.count ? item : max, ordersChart[0])
      : null;
    
    // Calculate customer satisfaction score (out of 5)
    const satisfactionScore = positiveRate / 20; // Convert percentage to 5-star scale
    
    // Calculate loyalty rate (customers with positive reviews)
    const loyaltyRate = positiveRate > 80 ? 95 + (positiveRate - 80) / 4 : positiveRate * 1.2;
    
    // Determine overall rating
    const getOverallRating = () => {
      if (positiveRate >= 90) return 'ممتاز';
      if (positiveRate >= 80) return 'جيد جداً';
      if (positiveRate >= 70) return 'جيد';
      if (positiveRate >= 60) return 'مقبول';
      return 'يحتاج تحسين';
    };
      ...
      <div className="min-h-[70vh] flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-purple-50">
        <div className="text-center space-y-8">
          ...
          <div className="relative">
            <div className="w-20 h-20 border-4 border-blue-200 border-t-blue-600 rounded-full animate-spin shadow-2xl"></div>
            <div className="absolute inset-0 w-20 h-20 border-4 border-transparent border-r-purple-600 rounded-full animate-spin shadow-2xl" style={{ animationDirection: 'reverse', animationDuration: '1.5s' }}></div>
            <div className="absolute inset-2 w-16 h-16 border-2 border-transparent border-b-pink-500 rounded-full animate-spin" style={{ animationDuration: '2s' }}></div>
            
            {/* Pulsing Center */}
            <div className="absolute inset-6 w-8 h-8 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full animate-pulse shadow-lg"></div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-purple-50/30 p-8 space-y-12">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
        {/* Orders Chart */}
        <ChartCard 
          title="تحليل الطلبات الشهرية" 
          subtitle="نمو وتطور المبيعات عبر الزمن"
          delay={400} 
          icon={BarChart}
        >
        ...
        {/* Reviews Pie Chart */}
        <ChartCard 
          title="توزيع التقييمات التفاعلي" 
          subtitle="نسب الرضا والتحسينات المطلوبة"
          delay={500} 
          icon={Eye}
        >
      ...
      {/* Reviews Trend Chart */}
      <ChartCard 
        title="اتجاهات التقييمات المتقدمة" 
        subtitle="تحليل شامل للتقييمات الإيجابية والسلبية"
        delay={600} 
        icon={TrendingUp}
      >
        <div className="h-96 group-hover:scale-[1.01] transition-transform duration-700 transform-gpu">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data.reviewsChart} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
              <defs>
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <div className="space-y-4">
              <MetricCard
                label="متوسط الطلبات"
                value={metrics?.avgOrdersPerMonth || 0}
                icon={ShoppingCart}
                color="text-emerald-600"
                delay={800}
              />
              <MetricCard
                ...
                value={metrics?.avgReviewsPerMonth || 0}
                icon={Star}
                color="text-amber-600"
                delay={900}
              />
                      ...
                      {[...Array(5)].map((_, i) => (
                        <Star 
                          ...
                          className={`w-5 h-5 transition-all duration-300 ${
                            i < Math.round((metrics?.satisfactionScore || 0)) 
