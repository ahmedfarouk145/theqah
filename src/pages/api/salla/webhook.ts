...
import { sendEmailDmail as sendEmail } from "@/server/messaging/email-dmail"; // متروك للتوافق إن حبيت تستخدمه
import { sendSms, buildInviteSMS } from "@/server/messaging/send-sms";       // متروك للتوافق
...
function extractProvidedToken(req: NextApiRequest): string {
  ...
  const h1 = getHeader(req, "x-salla-token").trim();
  const h2 = getHeader(req, "x-webhook-token").trim();
  if (h1) return h1;
  if (h2) return h2;
...
function validateCustomerData(customer?: SallaCustomer): {
  ...
  return { isValid: hasEmail || hasMobile, hasEmail, hasMobile, email: hasEmail ? email : undefined, mobile: hasMobile ? mobile : undefined };
}
...
/** ✅ تكتب الاسكيمة الجديدة + حقول التوافق القديمة */
async function maybeSaveDomain(
  ...
  await db.collection("stores").doc(uid).set({
    uid,
    provider: "salla",
    domain: { base, key, updatedAt: Date.now() }, // canonical
  ...
  await db.collection("domains").doc(key).set({
    base, key,
    uid,               // canonical field
    storeUid: uid,     // legacy field (مستخدم في /resolve)
    provider: "salla",
    updatedAt: Date.now(),
  }, { merge: true });
...
async function ensureInviteForOrder(
  ...
  // customer من كل المصادر المحتملة
  ...
  const cv = validateCustomerData(customer);
  if (!cv.isValid) {
    console.log("[SALLA][INVITE] skip - no valid customer contact info", { orderId, customer });
    return;
  }

  // Idempotency: لو فيه دعوة للطلب ده، خلاص
  ...
  const base =
    (process.env.APP_BASE_URL ||
      ...
      "").replace(/\/+$/, "");
  if (!base) throw new Error("BASE_URL not configured (APP_BASE_URL / NEXT_PUBLIC_APP_URL / NEXT_PUBLIC_BASE_URL)");
...
async function voidInvitesForOrder(db: FirebaseFirestore.Firestore, orderId: string, reason: string): Promise<void> {
  if (!orderId) return;
  const q = await db.collection("review_tokens").where("orderId", "==", orderId).get();
  if (q.empty) {
    console.log("[SALLA][VOID] no tokens for order", { orderId });
    return;
  }
  const batch = db.batch();
  q.docs.forEach((doc) => {
    batch.update(doc.ref, { voidedAt: Date.now(), voidReason: reason });
  });
  await batch.commit();
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!verification.ok) {
    console.error("[SALLA][AUTH] invalid request", {
      ...
      body: raw.toString("utf8").slice(0, 500),
      reason: "signature/token mismatch or missing",
    });
    return res.status(401).json({ error: "unauthorized" });
  }

  // Fast ACK (202) — بنعالج في الخلفية
  ...
  try {
    ...
    console.error("[SALLA][PARSE] invalid JSON", {
      ...
      rawLen: raw.length,
      raw: raw.toString("utf8").slice(0, 1000),
    });
    await db.collection("webhook_errors").add({
      at: Date.now(), scope: "parse", error: "invalid_json",
      headers: req.headers, rawLen: raw.length, raw: raw.toString("utf8").slice(0, 2000),
    }).catch(() => {});
    return;
  }
  ...
  // Idempotency (مفتاح مبني على التوقيع + الجسم)
  try {
    const sigHeader = getHeader(req, "x-salla-signature") || "";
    ...
    if ((await idemRef.get()).exists) {
      console.log("[SALLA][IDEMP] duplicate request", { event });
      return;
    }
    await idemRef.set({
      at: Date.now(),
    ...
    console.error("[SALLA][IDEMP] failed", { error: error instanceof Error ? error.message : String(error) });
  }

  // حفظ الدومين للأحداث ذات الصلة (ومنها app.installed اللي اديتني payload بتاعه)
  if (event === "app.store.authorize" || event === "app.installed") {
    ...
    if (uid !== "salla:unknown") {
      await maybeSaveDomain(db, uid, dataRaw, event).catch((error) => {
        console.error("[SALLA][DOMAIN] save failed", { event, uid, error: error instanceof Error ? error.message : String(error) });
      });
    }
  }
  ...
  try {
    ...
    // حفظ سنابشوت الطلب في أحداث الطلب/الشحن
    ...
    console.log("[SALLA][OK] processed", { event, orderId, status });

  } catch (processingError) {
    const errorMessage = processingError instanceof Error ? processingError.message : String(processingError);
    console.error("[SALLA][ERROR] processing failed", {
      event, error: errorMessage, stack: processingError instanceof Error ? processingError.stack : undefined
    });
    await db.collection("webhook_errors").add({
      at: Date.now(),
      scope: "handler",
