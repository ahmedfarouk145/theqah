...
// Activity analytics endpoint - wrapper around activity-tracker module
  ...
  try {
    switch (action) {
      ...
      case 'feature_usage': {
        ...
        return res.status(200).json({ usage: Object.fromEntries(usage), startDate, endDate });
      }
      ...
      case 'retention': {
        if (!cohortStart || !cohortEnd || !checkDate) {
          return res.status(400).json({ error: 'cohortStart, cohortEnd, and checkDate required' });
        }
        ...
        return res.status(200).json({ retentionRate: rate, cohortStart, cohortEnd, checkDate });
      }
