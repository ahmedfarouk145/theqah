...
export default function ReviewsTab({ storeName }: { storeName?: string }) {
  ...
  // Download export with auth token
  const handleExport = async (type: 'csv' | 'pdf') => {
    if (!user) {
      setError('الرجاء تسجيل الدخول للتصدير');
      return;
    }
          ...
          <div className="mt-8 flex gap-2">
            {tabs.map((t) => (
              <button
                ...
                className={`flex items-center gap-3 px-6 py-3 rounded-2xl font-bold transition-all duration-300 transform hover:scale-105 ${tab === t.id
