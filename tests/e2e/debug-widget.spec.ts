...
test('debug widget on perfumedeer', async ({ page }) => {
    ...
    const productUrl = await page.evaluate(() => {
        ...
        // Look for common Salla/Zid product URL patterns
        const productLink = links.find(a =>
            a.href.includes('/p/') ||
            a.href.includes('/products/') ||
            (a.href.match(/-c\d+/) && !a.href.includes('category'))
        );
    ...
    if (productUrl) {
        ...
        const widgetInfo = await page.evaluate(() => {
            ...
            return {
                ...
                widgetHtml: widgetEl ? widgetEl.outerHTML.substring(0, 500) : 'N/A', // Increased limit
                iframes_related: iframes.filter(src => src.includes('theqah'))
            };
        });
    ...
    // Take a screenshot of the potential widget area
    await page.screenshot({ path: 'debug-perfumedeer.png', fullPage: true });
});
